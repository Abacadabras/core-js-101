...
function isBracketsBalanced(str) {
  ...
  for (const symbol of str) {
    if (openSymbols.includes(symbol)) {
      ...
      const pairSymbol = openSymbols[index];
      if (stack.pop(pairSymbol) !== pairSymbol) return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  const commonPath = arrPath.reduce((acc, path) => acc.filter((part, i) => part === path[i]));
...
function getMatrixProduct(m1, m2) {
  ...
  return result
    .map((row, i) => row.map((val, j) => m1[i].reduce((sum, elm, k) => sum + (elm * m2[k][j]), 0)));
}
